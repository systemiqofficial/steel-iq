...
def test_collect_active_energy_subsidies_includes_all_technology():
    """Test that technology_name='all' subsidies are merged with tech-specific ones."""
    tech_sub = Subsidy(
        scenario_name="test",
        iso3="DEU",
        start_year=Year(2025),
    ...
    energy_subsidies = {
        "electricity": {"DEU": {"DRI": [tech_sub], "all": [all_sub]}},
    }
    result = calculate_costs.collect_active_energy_subsidies(
        energy_subsidies,
        "DEU",
        "DRI",
        Year(2026),
    ...
    assert tech_sub in result["electricity"]
    assert all_sub in result["electricity"]


def test_collect_active_energy_subsidies_all_only():
    """Test that 'all' subsidies are returned even when no tech-specific ones exist."""
    all_sub = Subsidy(
        scenario_name="test",
        iso3="DEU",
        start_year=Year(2025),
    ...
    energy_subsidies = {
        "natural_gas": {"DEU": {"all": [all_sub]}},
    }
    # BF has no tech-specific subsidies, but "all" should still match
    result = calculate_costs.collect_active_energy_subsidies(
        energy_subsidies,
        "DEU",
        "BF",
    ...
    assert all_sub in result["natural_gas"]


def test_collect_active_energy_subsidies_filters_by_year():
    """Test that only active subsidies for the given year are returned."""
    expired_sub = Subsidy(
        scenario_name="test",
        iso3="DEU",
    ...
    energy_subsidies = {
        "electricity": {"DEU": {"all": [expired_sub]}},
    }
    result = calculate_costs.collect_active_energy_subsidies(
        energy_subsidies,
        "DEU",

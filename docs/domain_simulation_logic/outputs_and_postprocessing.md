...
| `embedded_emissions_map.html` | The emissions embedded in steel trade as a world map with a year slider: each furnace group's direct and indirect emissions (the run's chosen emissions boundary) are carried along the realised allocations, iron furnace group → steel furnace group → demand centre, pro rata to volume, giving production-based and consumption-based emissions per country, the emissions embedded in imports and exports, net imports (also as % of production-based) and the embedded intensity of the steel consumed. Clicking a country shows its series and partners; the geography filter treats a selection as one economy, netting out trade inside it. Indirect emissions are grid electricity at the plant's location plus the technology's indirect factor | post-processed CSV + `TM/steel_trade_allocations_<year>.csv` |
...
| `capacity_world_map.html` | One pie per plant on a world map (area = capacity, wedges = technology) with a year slider, filters for technology, status (operating, construction), plant origin (existing fleet, greenfield) and geography (regions, countries, China by province), a statistics side panel and a PNG export; pies that overlap at the current zoom merge. Groups being rebuilt for a technology switch show as under construction in the new technology | `post_processed_<timestamp>.csv` (operating groups) + `data/greenfield_status_timeseries.csv` (new builds under construction) + `data/pam_switch_decisions.csv` (rebuild years) + `data/pipeline_status_timeseries.csv` (the existing fleet's groups that are not operating yet: the input data's pipeline units and the PAM's expansions) + `data/pam_motions.csv` (fallback for runs without the pipeline table: expansions under construction from the motion year to their first operating year) + the run's live plants (coordinates, origin) |

...
class SteelPlotter:
    ...
    Class attributes:
        EMISSIONS_SUBDIR: Subdirectory under ``pam_plots_dir`` where emission charts are
            written. Kept centralised so call sites and tests reference one symbol.
    ...
    def _plot_single_scope_emissions(
        ...
        if export_csv:
            self._save_chart_data_to_csv(pivot, filename)
        return self._save_figure(fig, filename)
    ...
    def _plot_combined_scope_emissions(
        ...
        if export_csv:
            self._save_chart_data_to_csv(combined, filename)
        return self._save_figure(fig, filename)
